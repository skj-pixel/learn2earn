...
def tx(s,l,t,w,h,txt,sz=14,c=DARK_TEXT,*,b=False,a=PP_ALIGN.LEFT,ac=MSO_ANCHOR.TOP,ls=1.3):
...
def TR(s,l,t,w,h,f):return shp(s,l,t,w,h,f,None,MSO_SHAPE.TRAPEZOID)
...
def tbl(s,l,t,w,h,data,cw=None,hc=INDIGO,ht=WHITE,bt=DARK_TEXT,alt=LIGHT_INDIGO,dark=False,fs=12):
    rs=len(data);cs=len(data[0]);t=s.shapes.add_table(rs,cs,l,t,w,h).table
    if cw:
        S=sum(cw)
        for i,c in enumerate(cw):t.columns[i].width=int(w*c/S)
    for ri,row in enumerate(data):
        for ci,v in enumerate(row):
            ce=t.cell(ri,ci);ce.margin_left=P(6);ce.margin_right=P(6);ce.margin_top=P(4);ce.margin_bottom=P(4)
            ...
            p=tf.paragraphs[0];p.alignment=PP_ALIGN.CENTER if (ci==0 or ri==0) else PP_ALIGN.LEFT;p.line_spacing=1.2
